...
// Stylized cold-boot connection screen with partial-update progress.
//
...
// that, stageStart()/stageDone()/tick() redraw just the status band and
// partial-refresh it, so the connect sequence shows live progress
// without flashing the whole panel.
//
