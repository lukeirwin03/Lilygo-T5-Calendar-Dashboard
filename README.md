...
- **Scroll arrows** — small chevron buttons (24 px visuals, much larger tap zones) appear at the top-right/bottom-right of today's timeline only when events are clipped off that edge. Tap ▲ to page back to scrolled-off past events, ▼ to return toward the live window; the offset resets on the next data refresh or navigation. Scrolling renders with a fast flash-free refresh of the focus column — with `GRAY_DIFF_ENABLED` (default) a gray differential swap with no ghost copies, plus a cleared reflash of the focus rows every 3rd scroll to scrub accumulated ghosting (`SCROLL_FLASH_EVERY`); the legacy no-clear ghost refresh (prior frames leave faint ghosts until the next full refresh) is the kill-switch fallback
...
- Detail transitions refresh only the right column (~2s instead of ~5s full refresh) — a gray differential swap with `GRAY_DIFF_ENABLED`, otherwise the cleared **partial refresh**
...
Day navigation (context-column taps and the daily ◀/▶ arrows) swaps the whole screen as a gray differential — the heaviest gray diff in the app — so it rides an every-3rd full-flash cadence (`NAV_FLASH_EVERY`) to scrub ghosting.
...
- Partial refresh comes in three flavors. The raw epdiy driver assumes the panel is **white** when drawing (`BLACK_ON_WHITE`, no previous-frame tracking): it can drive pixels toward black/gray, but a **white target produces no drive** — a plain no-clear push can only *add* ink. **Ghost refresh** (`ghostRefresh`, timeline scrolling): exactly that — fast and flash-free, but old content stays at full strength. **Cleared partial refresh** (daily↔detail): `epd_clear_area` inverts the region dark-then-white before drawing — the brief flash *is* the erase. **Differential refresh** (`diffRefresh`, connection screen, settings modal): flash-free full-swap — the app tracks what ink is physically on the panel, then per update: one `WHITE_ON_WHITE` "white ink" pass erases *all* the old ink (glyph-masked — paper/background pixels are never driven), no settle gap, and two fast 1-bit passes ink all the new content (~half a second per update, no flashing). Hardware-characterized on this panel via the demo build's calibration screen: a single erase pass is the correct dose (more passes build charge and the erased pixels fade back in), whole-band erases grey the background, and gap-0 two-pass-ink swaps are clean. This gets Kindle-like silent updates out of dumb glass — the driver has the white-ink mode, it just has no memory of what was drawn, so the app supplies it. The differential layer is exposed as `display_mgr::DiffRegion` — an object that owns the previous-frame record for a rect (see `display_manager.h`) — with the raw passes still available for calibration. The settings modal rides the same engine (`ui_settings::diffPush()`/`closeReflash()`): open, tab switch, and row edits are all flash-free and column-limited — opening runs one gentle white pass over ONLY the modal's columns (the strips of underlying view beside it keep their ink) that also wipes any ghost-scroll residue underneath — and close is a localized cleared reflash of the modal's row range with fresh data (resetting the pixels the diff drives touched). Because the 1-bit ink path thresholds grays to black, the modal renders binary — hence its inverted (black/white) selected rows and solid-black buttons. Gray content can also swap flash-free (`DiffRegion::updateGray()` — white-ink erase + 4-bit draw), pending hardware validation via the GRAY SWAP calibration row. The calendar views ride that gray variant: day navigation, daily-detail open/close, and timeline scrolls push as gray diffs (`ui::pushViewGray()` and friends) — silent, no flash, no ghost copies — with two flash carve-outs driven by hardware feedback: view switches (weekly↔daily) flash every time (the whole-screen gray swap ghosted, and the flash reads as intentional on so big a content change), and timeline scrolls get a cleared reflash of the focus rows every 3rd arrow tap (`SCROLL_FLASH_EVERY` — repeated same-column gray swaps accumulate ghosting fastest). All of it is governed by a refresh-hygiene policy: each flash-free update earns a "credit" — gray diffs ghost more than binary ones, so the cap is deliberately tight; tune from serial observations — and once the cap (12) is hit, or after 15 minutes of an awake session, or 24 h absolute (persisted across deep sleep in RTC memory), the NEXT user-initiated transition silently upgrades to a full cleared flash instead of another diff (never a spontaneous mid-interaction flash; wake renders are always full flashes, which reset everything). `GRAY_DIFF_ENABLED` in `config.h` is the kill-switch: set it false to restore the legacy flashing/ghost behavior everywhere while the gray swap awaits hardware validation.
